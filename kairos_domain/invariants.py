...
__all__ = ["Enforcement", "Invariant", "INVARIANTS", "unenforced", "by_module"]
...
INVARIANTS: tuple[Invariant, ...] = (
    Invariant(1, "O prefixo de cache não muda mid-conversa (exceto compactação)",
              "agent", Enforcement.DEFERRED, "Tarefa 13 — agent"),
    Invariant(2, "Alternância estrita de papéis; nunca duas do mesmo papel seguidas",
              "agent", Enforcement.DOMAIN, "message.check_role_alternation"),
    Invariant(3, "Nunca uma mensagem de usuário sintética injetada mid-loop",
              "agent", Enforcement.DOMAIN, "message.check_no_synthetic_user_message"),
    Invariant(4, "Compactação nunca corta um par tool_call/tool_result",
              "agent", Enforcement.DOMAIN, "message.check_tool_pairs_intact + compaction.plan_boundary"),
    ...
    Invariant(8, "'Abandonado' só com prova de morte (PID + hora de início)",
              "cron", Enforcement.DOMAIN, "scheduling.finish_status_for_dead_owner"),
    Invariant(9, "Atores autônomos nunca fazem hard-delete",
    ...
    Invariant(13, "Caminhos sensíveis pedem aprovação mesmo sob política autônoma",
              "acp_adapter", Enforcement.DEFERRED, "Tarefa 17 — acp-adapter (T-15)"),
    Invariant(14, "Exceção no aprovador = negação (fail-closed)",
              "acp_adapter", Enforcement.DEFERRED, "Tarefa 17 — acp-adapter"),
    Invariant(15, "Um job com subprocesso morto nunca reporta sucesso",
              "cron", Enforcement.DOMAIN, "scheduling.assert_not_reporting_success_when_dead"),
)
