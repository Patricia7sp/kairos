...
import upstream classes into Kairos itself.
...
```bash
mkdir -p ~/.kairos/cache/darwinian-evolver && cd ~/.kairos/cache/darwinian-evolver
...
```bash
cd ~/.kairos/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.kairos/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.kairos/skills/research/darwinian-evolver
DE_DIR=~/.kairos/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Kairos core.
   Custom driver scripts under `~/.kairos/skills/...` are user-side and fine.
