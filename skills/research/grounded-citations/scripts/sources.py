...
Ledger path resolution (first wins):
  --ledger PATH
  $KAIROS_CITATION_LEDGER
  $KAIROS_HOME/cache/citations/ledger.json
"""
...
from _kairos_home import get_kairos_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("KAIROS_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_kairos_home() / "cache" / "citations" / "ledger.json"
