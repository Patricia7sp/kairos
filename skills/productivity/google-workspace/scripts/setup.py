#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Kairos.
...
# Ensure sibling modules (_kairos_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _kairos_home import display_kairos_home, get_kairos_home

KAIROS_HOME = get_kairos_home()
TOKEN_PATH = KAIROS_HOME / "google_token.json"
CLIENT_SECRET_PATH = KAIROS_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = KAIROS_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Kairos profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Kairos Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Kairos Docker image's "
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Kairos home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_kairos_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Kairos")
