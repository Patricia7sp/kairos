#!/usr/bin/env python3
"""Google Workspace API CLI for Kairos.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Kairos-facing JSON contract and falls back to the Python client
...
from _kairos_home import get_kairos_home
...
def _gws_binary() -> str | None:
    override = os.getenv("KAIROS_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Kairos")
