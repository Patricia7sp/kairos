...
This optional skill gives Kairos practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${KAIROS_HOME:-~/.kairos}/.env`
...
It does **not** turn Kairos into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Kairos memory
...
### 1) "I want Kairos to own a real phone number"
Use **Twilio**.
...
- pairs well with Kairos `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${KAIROS_HOME:-~/.kairos}/.env`
...
```bash
SCRIPT="$(find ~/.kairos/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
kairos skills search telephony
kairos skills install official/productivity/telephony
```
...
Then save credentials into Kairos:
...
3. Buy it and save it into `${KAIROS_HOME:-~/.kairos}/.env` + state:
```bash
python "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Kairos calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Kairos's existing `text_to_speech` support.

Use this when:
- you want the call to use Kairos's configured TTS voice rather than Twilio `<Say>`
...
Recommended Kairos TTS -> Twilio Play workflow:

1. Generate the audio with Kairos `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Kairos to create the message audio with `text_to_speech`
...
Important note:
- Kairos TTS is great for prerecorded outbound messages
...
- Kairos STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Kairos memory.
...
- Do not store arbitrary third-party phone numbers in Kairos memory.
...
3. persist that number to `${KAIROS_HOME:-~/.kairos}/.env`
