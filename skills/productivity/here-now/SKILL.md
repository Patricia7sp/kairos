...
- Skill helper paths:
  - `${KAIROS_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${KAIROS_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${KAIROS_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client kairos
```
...
```bash
PUBLISH="${KAIROS_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client kairos
```
...
```bash
DRIVE="${KAIROS_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `kairos`)    |
