...
| List/filter by metadata (age, source, cost, tokens, workspace) | `kairos sessions list` / `stats` via terminal |
| Rename | `kairos sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `kairos sessions archive <filters>` |
| Delete (destructive) | `kairos sessions delete` / `kairos sessions prune <filters>` |
| Export before deleting anything valuable | `kairos sessions export --session-id <id> --format md` |
...
`kairos sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `kairos sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `kairos sessions list`
