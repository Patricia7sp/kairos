...
def _check_config():
    ...
    if missing:
        kairos_env = os.path.join(
            os.environ.get("KAIROS_HOME", os.path.expanduser("~/.kairos")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {kairos_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Kairos"
    )
