...
Everything operator-facing is a `kairos teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${KAIROS_HOME:-~/.kairos}/.env`:
...
```bash
kairos teams-pipeline validate              # config snapshot — run first after any change
kairos teams-pipeline token-health          # Graph token status
kairos teams-pipeline token-health --force-refresh   # force a fresh token acquisition
kairos teams-pipeline list                  # recent meeting jobs
kairos teams-pipeline list --status failed  # only failed jobs
kairos teams-pipeline show <job-id>         # full detail of one job
kairos teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
kairos teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
kairos teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
kairos teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
kairos teams-pipeline fetch --join-web-url "<url>" --organizer-user-id <id>   # organizer-scoped lookup (required for /meet/ short URLs)
```

### Subscription management

```bash
kairos teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

kairos teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
kairos teams-pipeline delete-subscription <sub-id>
kairos teams-pipeline maintain-subscriptions            # renew near-expiry ones
kairos teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `kairos teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `kairos cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
