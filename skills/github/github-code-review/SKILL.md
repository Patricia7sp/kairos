...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _kairos_env="${KAIROS_HOME:-$HOME/.kairos}/.env"; [ -f "$_kairos_env" ] && grep -q "^GITHUB_TOKEN=" "$_kairos_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_kairos_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python "${KAIROS_HOME:-$HOME/.kairos}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
```bash
source "${KAIROS_HOME:-$HOME/.kairos}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Kairos. Code looks clean — good test coverage, no security concerns."
