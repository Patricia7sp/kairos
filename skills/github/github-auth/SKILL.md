...
- Give it a title like "kairos-agent-<machine-name>"
...
uv run python "${KAIROS_HOME:-$HOME/.kairos}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif _kairos_env="${KAIROS_HOME:-$HOME/.kairos}/.env"; [ -f "$_kairos_env" ] && grep -q "^GITHUB_TOKEN=" "$_kairos_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_kairos_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python "${KAIROS_HOME:-$HOME/.kairos}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
