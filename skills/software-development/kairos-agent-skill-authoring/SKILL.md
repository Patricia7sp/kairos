---
name: kairos-agent-skill-authoring
description: "Author in-repo SKILL.md files: frontmatter and structure."
version: 2.0.0
author: Kairos
license: MIT
platforms: [linux, macos, windows]
tags: [skills, authoring, kairos-agent, conventions, skill-md]
related_skills: [plan, requesting-code-review]
category: software-development
---
# Authoring Kairos-Agent Skills (in-repo)

## Overview

There are two places a SKILL.md can live:

1. **User-local:** `~/.kairos/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the kairos-agent repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.
...
- You're committing a reusable workflow that should ship with kairos-agent
...
- Don't use for: personal skills in `~/.kairos/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `kairos skills install official/<category>/<skill>`.
...
author: Real Name (github-handle), Kairos
license: MIT
...
- Credit the **human first**, then "Kairos" as secondary collaborator: `Ben Barclay (benbarclay), Kairos`.
- Never `author: Kairos` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), Kairos`.

### `related_skills` rules

- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.kairos/skills/`.
...
|---|---|
| Kairos tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.kairos/skills/`, not the repo tree. Use `write_file`.
...
3. **`author: Kairos` on a contributed skill.** Credit the human first.
...
- [ ] `name`, `description`, `version`, `author`, `license`, `platforms`, `metadata.kairos.{tags, related_skills}` all present
...
- [ ] Body follows the modern section order; commands framed through Kairos tools
