...
├── cli.py                # Interactive CLI (KairosCLI)
├── kairos_state.py       # SQLite session store
├── agent/                # Prompt builder, context compression, memory, model routing, credential pooling, skill dispatch
├── kairos_cli/           # CLI subcommands, config, setup, commands
...
Config: `~/.kairos/config.yaml` (settings), `~/.kairos/.env` (API keys) — both under `$KAIROS_HOME` when it is set.
...
`_KAIROS_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_kairos_home()` for paths,
never hardcode `~/.kairos`. For custom/local-only tools, write a plugin in
`~/.kairos/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `kairos_cli/commands.py`
...
- Tests auto-redirect `KAIROS_HOME` to temp dirs — never touch real `~/.kairos/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/kairos_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_kairos_home()` from `kairos_constants` for all paths (profile-safe)
