# Themes / Skins — Author a Kairos Color Theme

Author a Kairos **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`kairos_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.kairos/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Kairos itself.
...
- Write access to the Kairos home dir — `~/.kairos` by default, or `$KAIROS_HOME`
  / the active profile's dir. Skins live in `<kairos-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `kairos config set`).
...
3. `write_file` it to `<kairos-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `kairos config set display.skin default` (or they can `/skin default`).
...
```
kairos skin set <key> <hex>      # e.g. kairos skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `kairos skin set background "#08201f"` changes
only the background; `kairos skin use <name>` / `kairos skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.kairos`** when a profile is active — resolve the real home
  from `$KAIROS_HOME` first, falling back to `~/.kairos`.
...
- **Never hand-edit `config.yaml` to activate.** Use `kairos config set
...
- **You apply it, not the user.** `kairos config set display.skin <name>` is
...
- `read_file` the written `<kairos-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `kairos config get display.skin` and confirm it reports `<name>`.
