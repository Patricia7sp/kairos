...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Kairos agent runs by POSTing events to a URL.
...
```bash
kairos gateway setup
```
Follow the prompts to enable webhooks, set the port, and set a global HMAC secret.

### Option 2: Manual config
Add to `~/.kairos/config.yaml`:
```yaml
platforms:
  webhook:
    enabled: true
...
All management is via the `kairos webhook` CLI command:

### Create a subscription
```bash
kairos webhook subscribe <name> \
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.kairos/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.kairos/scripts/` (path traversal is blocked).

```bash
kairos webhook subscribe todoist-kairos \
  --prompt "Task changed: {payload.content}" \
  --script "todoist-kairos-label.py" \
...
```bash
kairos webhook remove <name>
```

### Test a subscription
```bash
kairos webhook test <name>
kairos webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: new issues
```bash
kairos webhook subscribe github-issues \
  --events "issues" \
...
### GitHub: PR reviews
```bash
kairos webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
kairos webhook subscribe stripe-payments \
...
```bash
kairos webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
kairos webhook subscribe alerts \
...
```bash
kairos webhook subscribe antenna-matches \
  --deliver telegram \
...
- Subscriptions persist to `~/.kairos/webhook_subscriptions.json`

## How It Works

1. `kairos webhook subscribe` writes to `~/.kairos/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status kairos-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.kairos/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `kairos webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `kairos webhook test <name>` to verify the route works.
