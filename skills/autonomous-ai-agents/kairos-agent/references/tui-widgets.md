...
Author widget apps for the Kairos TUI (`kairos --tui`): glanceable ambient
...
- The TUI must be in use (`kairos --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.kairos/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.kairos/logs/tui_gateway_crash.log` if a widget never appears.
