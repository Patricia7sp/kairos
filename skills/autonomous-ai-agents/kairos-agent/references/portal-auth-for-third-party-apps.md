...
## Layer 1 — Is this thing a Kairos plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Kairos process**. Its LLM calls go through Kairos's already-configured provider. | Already uses Portal if user's Kairos is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Kairos. That LLM call is already authenticated
via Kairos's provider config. The plugin's own server auth (e.g.
...
What Kairos has that *feels* like OAuth — `kairos login --provider nous`
opening a browser, user signs in, token lands in `~/.kairos/auth.json` — is a
**Kairos-specific browser flow**. Under the hood it produces a credential
Kairos uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Kairos's existing Portal credential out of `~/.kairos/auth.json`.
...
rides on the credential Kairos already holds.

Where this could live in Kairos:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Kairos itself.
...
1. First decide: Kairos plugin (runs inside Kairos) or separate app? If plugin,
   it already uses Portal via Kairos's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Kairos agent itself") is fully supported. Kairos treats agent
...
pitch in the Tool Gateway announcement is specifically about Kairos's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Kairos is configured to use Portal as its provider. It is
