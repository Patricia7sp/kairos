...
Set via `kairos model` (picker) or `kairos setup`. 35+ provider profiles ship as
...
| nous | OAuth device code | `kairos auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `kairos auth add openai-codex` |
| qwen-oauth | OAuth | `kairos auth add qwen-oauth` |
| minimax-oauth | OAuth | `kairos auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`kairos auth`).
Fallback chain when the primary fails: `kairos fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`kairos_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   kairos config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
