...
Write plugins for the Kairos desktop app: statusbar items, layout panes,
...
system (`~/.kairos/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$KAIROS_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$KAIROS_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Kairos desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$KAIROS_HOME/desktop-plugins/` (usually
  `~/.kairos/desktop-plugins/`).

## How to Run

1. Create `$KAIROS_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.kairos/...` by default, or `~/.kairos/profiles/<profile>/...` under a
...
The ONLY import surface is `@kairos/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Kairos
  ...
  (`kairos://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.kairos/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@kairos/plugin-sdk`, `react`, and
