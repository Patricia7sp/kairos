...
Edit with `kairos config edit` or `kairos config set section.key value`.
...
`kairos config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `kairos tools` (interactive) or `kairos tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_KAIROS_CORE_TOOLS` is the default bundle most platforms inherit).
