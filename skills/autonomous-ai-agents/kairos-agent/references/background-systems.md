...
the `cronjob` tool, the `kairos cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `kairos curator <verb>` — `status`, `usage`, `run`, `pause`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `kairos curator run --consolidate`. Routine background curation costs
  zero tokens.
- **Telemetry:** sidecar at `~/.kairos/skills/.usage.json` holds
...
Users drive it via `kairos kanban <verb>`; dispatcher-spawned workers
see a focused `kanban_*` toolset gated by `KAIROS_KANBAN_TASK`, and
...
- **Isolation:** board is the hard boundary (workers get
  `KAIROS_KANBAN_BOARD` pinned in env); tenant is a soft namespace
