---
name: honcho
description: Configure and troubleshoot Honcho memory for Kairos.
version: 2.0.0
author: Kairos
license: MIT
...
---
# Honcho Memory for Kairos

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Kairos profile its own peer identity while sharing a unified view of the user.
...
```bash
kairos honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Kairos profile's AI peer
...
Honcho models conversations as interactions between **peers**. Kairos creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Kairos instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Kairos run |
| `global` | Single session across all directories |

Manual override: `kairos honcho map my-project-name`
...
Each Kairos profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
kairos profile create coder --clone
# creates host block kairos.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `kairos.coder` host block in `honcho.json`
...
```bash
kairos honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Kairos when Honcho memory is active.
...
Config file: `$KAIROS_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `kairos honcho setup`. Ensure `memory.provider: honcho` is in `~/.kairos/config.yaml`.

### Memory not persisting across sessions
Check `kairos honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `kairos profile create <name> --clone`. For existing profiles: `kairos honcho sync`.
...
| Command | Description |
|---------|-------------|
| `kairos honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `kairos honcho status` | Show resolved config, connection test, peer info for active profile |
| `kairos honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `kairos honcho disable` | Disable Honcho for the active profile |
| `kairos honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `kairos honcho peers` | Show peer identities across all profiles |
| `kairos honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `kairos honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `kairos honcho sessions` | List known directory-to-session-name mappings |
| `kairos honcho map <name>` | Map current working directory to a Honcho session name |
| `kairos honcho identity` | Seed AI peer identity or show both peer representations |
| `kairos honcho sync` | Create host blocks for all Kairos profiles that don't have one yet |
| `kairos honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Kairos + Honcho |
| `kairos memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `kairos memory status` | Show active memory provider and config |
| `kairos memory off` | Disable external memory provider |
