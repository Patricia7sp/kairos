...
version: 0.1.1
author: Matt Maximo (MattMaximo), Kairos
license: MIT
platforms: [linux, macos, windows]
tags: [Coding-Agent, Grok, xAI, Code-Review, Refactoring, Automation]
related_skills: [codex, claude-code, kairos-agent]
category: autonomous-ai-agents
---
# Grok Build CLI — Kairos Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Kairos terminal. Grok
...
1. Prepare stable input files first with Kairos tools (`read_file`,
...
2. **Don't conflate Kairos' xAI auth with the `grok` CLI's auth.** Kairos'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Kairos' xAI auth carries
   over.
