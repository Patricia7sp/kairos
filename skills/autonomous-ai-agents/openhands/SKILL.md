...
version: 0.1.0
author: Tim Koepsel (xzessmedia), Kairos
license: MIT
platforms: [linux, macos]
tags: [Coding-Agent, OpenHands, Model-Agnostic, LiteLLM]
related_skills: [claude-code, codex, opencode, kairos-agent]
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Kairos.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Kairos-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `kairos-agent` (Kairos subagents via `delegate_task`).
