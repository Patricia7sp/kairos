...
version: 0.1.0
author: Hugo Sequier (Hugo-SEQUIER), Kairos
license: MIT
...
The script reads `${KAIROS_HOME:-~/.kairos}/.env` for two optional defaults:
...
Helper script: `~/.kairos/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python ~/.kairos/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${KAIROS_HOME:-~/.kairos}/.env`.
...
```bash
python ~/.kairos/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python ~/.kairos/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
