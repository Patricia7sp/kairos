...
All per-subject data lives under PDD_DATA_DIR (default: $KAIROS_HOME/unbroker),
which is the same trust boundary Kairos uses for .env and OAuth tokens.
"""
...
def kairos_home() -> Path:
    return Path(os.environ.get("KAIROS_HOME") or (Path.home() / ".kairos"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else kairos_home() / "unbroker"
