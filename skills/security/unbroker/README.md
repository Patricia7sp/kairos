...
The design is **Kairos-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
kairos skills install official/security/unbroker
```

Then start a new Kairos session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Kairos session:
...
- **PII handling.** Dossiers live under the Kairos home directory (`0600`, optionally
