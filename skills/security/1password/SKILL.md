...
version: 1.0.0
author: arceus77-7, enhanced by Kairos
license: MIT
...
- `tmux` available for stable authenticated sessions during Kairos terminal calls (desktop app flow only)
...
### Service Account (recommended for Kairos)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${KAIROS_HOME:-~/.kairos}/.env` (the skill will prompt for this on first load).
...
## Kairos Execution Pattern (desktop app flow)

Kairos terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/kairos-tmux-sockets"
mkdir -p "$SOCKET_DIR"
