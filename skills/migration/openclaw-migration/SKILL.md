...
description: Import an OpenClaw setup (memories, skills) into Kairos.
version: 1.0.0
author: Kairos (Nous Research)
license: MIT
platforms: [linux, macos, windows]
tags: [Migration, OpenClaw, Kairos, Memory, Persona, Import]
related_skills: [kairos-agent]
category: migration
---
# OpenClaw -> Kairos Migration

Use this skill when a user wants to move their OpenClaw setup into Kairos with minimal manual cleanup.
...
```bash
kairos claw migrate              # Full interactive migration
kairos claw migrate --dry-run    # Preview what would be migrated
kairos claw migrate --preset user-data   # Migrate without secrets
kairos claw migrate --overwrite  # Overwrite existing conflicts
kairos claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `kairos setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_kairos.py` to:

- import `SOUL.md` into the Kairos home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Kairos memory entries
- merge OpenClaw command approval patterns into Kairos `command_allowlist`
- migrate Kairos-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Kairos working-directory configuration
- copy OpenClaw skills into `~/.kairos/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Kairos workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.kairos/tts/`
- archive non-secret docs that do not have a direct Kairos destination
...
- `scripts/openclaw_to_kairos.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.kairos/skills/migration/openclaw-migration/scripts/openclaw_to_kairos.py`

Do not guess a shorter path like `~/.kairos/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.kairos/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Kairos-compatible secrets, currently:
...
kairos CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Kairos. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Kairos workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Kairos skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.kairos/skills/migration/openclaw-migration/scripts/openclaw_to_kairos.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.kairos/skills/migration/openclaw-migration/scripts/openclaw_to_kairos.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.kairos/skills/migration/openclaw-migration/scripts/openclaw_to_kairos.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.kairos/skills/migration/openclaw-migration/scripts/openclaw_to_kairos.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.kairos/skills/migration/openclaw-migration/scripts/openclaw_to_kairos.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.kairos/skills/migration/openclaw-migration/scripts/openclaw_to_kairos.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Kairos unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Kairos targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Kairos destination. Unsupported auth blobs must still be reported as skipped.
...
- Kairos persona state imported
- Kairos memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.kairos/skills/openclaw-imports/`
