...
the actual `setup.sh` that creates the project workspace, configures Kairos
...
2. **Create profiles** — `kairos profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.kairos/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `kairos kanban create` assigned to the director
...
```bash
kairos profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.kairos/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.kairos/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Kairos
...
Each profile gets a `SOUL.md` at `~/.kairos/profiles/<name>/SOUL.md` that
...
```bash
kairos kanban create "Direct production of <video title>" \
    --assignee director \
...
the Kairos `.env` (`${KAIROS_HOME:-$HOME/.kairos}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _kairos_env="${KAIROS_HOME:-$HOME/.kairos}/.env"
    if grep -q "^${var}=" "$_kairos_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_kairos_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_kairos_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY kairos ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY kairos OPENROUTER_API_KEY || exit 1
# ...
```
