...
kairos kanban watch --tenant <project-slug>

# Snapshot of the board
kairos kanban list --tenant <project-slug>
kairos kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
kairos kanban stats --tenant <project-slug>

# Visual dashboard (browser)
kairos dashboard

# Inspect a specific task (includes comments + events)
kairos kanban show <task-id>

# Follow a single task's event stream
kairos kanban tail <task-id>
```

Verify available subcommands with `kairos kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `kairos kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `kairos kanban block <id>` then `kairos kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `kairos kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `kairos kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `kairos profile list` (profile exists?), `kairos status` (gateway/dispatcher up?), and verify tenant. |
...
kairos kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
kairos kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(kairos kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
kairos kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
kairos kanban block <task-id>
kairos kanban archive <task-id>

# Diagnose what happened
kairos kanban show <task-id>      # task body, comments, recent events
kairos kanban tail <task-id>      # follow the live event stream
kairos kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `kairos kanban create` for the director
...
while true; do
    clear
    kairos kanban list --tenant <slug>
    echo "---"
    kairos kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `kairos kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `kairos profile list` and start
