...
Maps each role archetype to the Kairos skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public kairos-agent
...
## Kairos skills relevant to video production

### Visual / rendering skills (`kairos-agent/skills/creative/`)
...
### Design / pre-production skills (`kairos-agent/skills/creative/`)
...
### Audio / media skills (`kairos-agent/skills/creative/` + `skills/media/`)
...
These are **not** Kairos skills but external CLIs / APIs that profiles invoke.
...
## Built-in Kairos tools for media review

These are native Kairos tools — not invoked via terminal but through their own
...
key is present in `${KAIROS_HOME:-~/.kairos}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Kairos profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${KAIROS_HOME:-~/.kairos}/.env` → environment variable.
...
When a new Kairos-public video skill ships:
