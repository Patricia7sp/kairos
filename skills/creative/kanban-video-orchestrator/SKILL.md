...
short to a music video to an ASCII loop — in a Kairos Kanban pipeline that
...
3. **Generates** a setup script that creates Kairos profiles, project workspace, and the initial kanban task
...
For mapping role → which Kairos skills + toolsets it loads, see
...
3. Creates each Kairos profile via `kairos profile create --clone`
...
7. Fires the initial `kairos kanban create` task assigned to the director
...
```bash
kairos kanban watch --tenant <project-tenant>     # live events
kairos kanban list  --tenant <project-tenant>     # board snapshot
kairos dashboard                                   # visual board UI
```
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `kairos kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${KAIROS_HOME:-~/.kairos}/.env` or the user's secret store.
