...
`$KAIROS_HOME/config.yaml` (defaults to `~/.kairos/config.yaml` when `KAIROS_HOME` is unset)
...
Restart the Kairos session for changes to take effect. The MCP connection is
