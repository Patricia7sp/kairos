...
KAIROS_HOME_DIR="${KAIROS_HOME:-$HOME/.kairos}"
KAIROS_CFG="${KAIROS_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Kairos config has twozero_td MCP entry ──
if [[ ! -f "$KAIROS_CFG" ]]; then
    echo -e " ${FAIL} Kairos config not found at ${KAIROS_CFG}"
    manual_steps+=("Create ${KAIROS_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$KAIROS_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Kairos config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Kairos config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${KAIROS_CFG} manually"); }
    manual_steps+=("Restart Kairos session to pick up config change")
fi
