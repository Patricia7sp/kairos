...
### 1. Start order: editor first, Kairos second

Kairos probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Kairos session.
...
restart the editor, restart the Kairos session.
...
server fails to bind (Output Log) or Kairos' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.kairos/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Kairos

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Kairos' connection
lives in `~/.kairos/config.yaml` via `kairos mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Kairos — just don't
mistake it for the Kairos setup step.
...
explicitly warns against overlapping calls. Kairos executes same-turn tool
...
### 9. Timeouts: Kairos gives up before Unreal does

Kairos' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Kairos runs elsewhere (SSH backend,
