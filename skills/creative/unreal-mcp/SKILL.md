...
Companion skill for the `unreal-engine` entry in the Kairos MCP catalog. The
...
Two halves, in this order: the editor side must be up before Kairos connects.
...
### One-time, Kairos side

    kairos mcp install unreal-engine
...
`~/.kairos/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Kairos — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Kairos connects
...
2. Start the Kairos session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Kairos
they appear as:
...
then happens at `kairos mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Kairos runs batched calls
...
- **Start order matters.** Editor + server up first, then the Kairos
...
- **Timeouts on long operations.** Kairos' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.kairos/config.yaml` for
