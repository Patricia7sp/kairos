...
related_skills: [kairos-agent, mcporter]
category: mcp
---
# FastMCP
...
- smoke-test a server with the FastMCP CLI before wiring it into Kairos or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Kairos. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.kairos/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.kairos/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Kairos integration, either:

- configure the server in `~/.kairos/config.yaml` using the `native-mcp` skill, or
...
### Kairos cannot see the deployed server

The server-building part may be correct while the Kairos config is not. Load the `native-mcp` skill and configure the server in `~/.kairos/config.yaml`, then restart Kairos.
