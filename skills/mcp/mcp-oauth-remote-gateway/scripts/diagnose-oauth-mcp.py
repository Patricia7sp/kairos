...
Usage:
  ...
  <server_name> matches the files in $KAIROS_HOME/mcp-tokens/<server>.json etc.
...
def _kairos_home():
    # Prefer Kairos' own resolver (profile-safe); fall back to env then ~/.kairos.
    try:
        from kairos_constants import get_kairos_home
        return str(get_kairos_home())
    except Exception:
        return os.environ.get("KAIROS_HOME") or os.path.expanduser("~/.kairos")


def _tokens_dir():
    return os.path.join(_kairos_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "kairos-diag", "version": "1.0"}},
        },
        headers={
