...
is typically only in `~/.kairos/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Kairos,
...
`kairos-custom-providers` skill, pitfall 2). Kairos hits this because it forwards
