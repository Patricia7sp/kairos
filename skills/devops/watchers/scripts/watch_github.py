...
Usage (via cron with --no-agent):
    ...
    kairos cron create kairos-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$KAIROS_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name kairos-issues --repo NousResearch/kairos-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Kairos .env file
(``${KAIROS_HOME:-~/.kairos}/.env``) to avoid the 60 req/hr
