...
Usage (via cron with --no-agent):
    ...
    kairos cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$KAIROS_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Kairos-Watcher/1.0"})
